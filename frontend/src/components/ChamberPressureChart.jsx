// frontend/src/components/ChamberPressureChart.jsx
...
export default function ChamberPressureChart() {
    ...
    const unsubscribe = subscribeEngineState((state) => {
      ...
      setSeries((prev) => {
        ...
        Object.entries(pressures).forEach(([name, value]) => {
          if (!name.toLowerCase().includes("chamber")) return;
  ...
  const chartData = {
    datasets: Object.entries(series).map(([name, data]) => ({
      label: `${name} (psi)`,
      data,
      borderWidth: 2,
      pointRadius: 0,
